...
module.exports = {
    ...
    createProduct: async (req, res, next) => {
        const {name,price,image,brand,category, description,rating, numReviews}= re.body;
        const product = new Product({name,price,image,brand,category, description,rating, numReviews});
        const newProduct= product.save();
        if(newProduct){
            return res.status(201).send({message:"Create product success",data:newProduct});
        }
